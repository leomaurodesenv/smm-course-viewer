...
class Draw {
    ...
    _drawObjects(objects) {
        ...
        $this._titleset.onload = function(){
            objects.forEach(function(courseObject) {
                ...
                if(blockTheme[courseObject.type]) { /* search in themes */
                    $this._drawObjectFromTheme(courseObject)
                }
                else {
                    //console.log('fault: '+courseObject.type);
                    $this._context.fillText(courseObject.type, x+3, y-3);
                }
            });
        };
    ...
    _drawObjectFromTheme(courseObject) {
        const $this = this;
        let cut = blockTheme[courseObject.type];
...
const blockTheme = {
    ...
    7: {'x': 8, 'y': 11, 'extend': function(courseObject) {
        ...
        else if(courseObject.childType == 4) { /* {!0} - ghost: flower */ /* ------------------------------------ check */
    ...
    14: {'x': 0, 'y': 0, 'extend': function(courseObject) {
        ...
        let width = courseObject.width;
        let height = courseObject.height;
        let color = (courseObject.flags >> 18) & 3; /* ------------------------------------ check: color and width */
