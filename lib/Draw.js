        ...
        if(cut.extend) { /* multiples cells */
            ...
            ext.forEach(function(drawExt) {
                let x = courseObject.x,
                    ...
                    yExt = drawExt.y;
                let objectPaint = {
                    'titleset': _titleset, 'xC': drawExt.xC, 'yC':drawExt.yC, 'xTs': _ts, 'yTs': _ts,
                    'x': x, 'y': y, 'xExt': xExt, 'yExt': yExt, 'xBase': base, 'yBase': base, 
                    'size': size, 'width': courseObject.width,
                };

                if(drawExt.text) $this._drawText(drawExt.x, drawExt.y, drawExt.text);
                else $this._paintObject(objectPaint);
            });
        }
        else { /* one cell */
            ...
            let objectPaint = {
                'titleset': _titleset, 'xC': xC, 'yC':yC, 'xTs': _ts, 'yTs': _ts,
                'x': x, 'y': y, 'xExt': 0, 'yExt': 0, 'xBase': base, 'yBase': base, 
                'size': 1, 'width': courseObject.width,
            };
...
var monsters_interpret = function(courseObject) {
    ...
    let type = ((flags & 0x7) & 4) >> 2,
        wing = ((flags & 0x7) & 2) >> 1,
        size = (flags & 0x4000) >> 14;
    // type = 0 (normal), 1 (wings)
    // size = 0 (normal), 1 (big)
    

    if(wing){
        let x = (courseObject.width - 1), xAdd = 0.7 * (size+1),
            y = (courseObject.height/(5.0));
        extend.push({'x': xAdd/(size+1), 'y': y, 'xC': 1, 'yC': 2});
        extend.push({'x': -(x+xAdd)/(size+1), 'y': y, 'xC': 0, 'yC': 2});
...
const blockTheme = {
    ...
    14: {'x': 0, 'y': 0, 'extend': function(courseObject) {
        ...
        if(width%2 == 0) {
            ...
            for(var h=0; h<(height-1); h++) {
                let w = (width-1)/2;
                if(h == 0) extend.push({'x': w, 'y': h, 'xC': 6, 'yC': 4});
