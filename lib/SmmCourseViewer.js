...
class _SmmCourseViewer {
        ...
        $this._interpreterCourse(_rawHex, function(eventInterpreter) {
            if(eventInterpreter) {
                ...
                for (let i = 0; i < $this.course.objectCount; i++) {
                    let pos = 0xF0 + (32 * i);
                    let courseObject = $this._interpreterObject(_rawHex, pos),
                        type = courseObject.type;
                    /* if is Block */
                    if(BlockObject.isBlock(type)) { 
                        let typedObject = new BlockObject(courseObject);
                        $this.objects.push(typedObject);
                    }
    ...
    _interpreterObject(_rawHex, _pos) {
        ...
        /* course object */
        x = parseInt(x/160); z = parseInt(z/160); y = parseInt(y/160); /* normalization */
        let objectData = {
