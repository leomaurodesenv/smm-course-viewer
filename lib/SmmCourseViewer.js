...
class _SmmCourseViewer {
    ...
    read(_file, _callback=function(){}) {
        ...
        $this._file = _file;
        /* start read the file */
        $this._readBinaryFile($this._file, function(err) {
            if(err) _callback(true);
            else _callback(false, $this.course, $this.objects);
        });
    }
    ...
    _interpreterBinaryFile(_rawHex, _callback) {
        const $this = this;
        $this._courseInterpreter(_rawHex, function() {
 ...
 */
try {
    var _smmCourseViewer = _SmmCourseViewer;
