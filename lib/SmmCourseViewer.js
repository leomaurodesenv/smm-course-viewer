...
class _SmmCourseViewer {
    ...
    read(_file, _callback=function(){}) {
        ...
        $this._readBinaryFile($this._file, function(eventRead) {
            if(eventRead) {
                console.log('SmmCourseViewer: finish - ok');
                _callback(true, $this.course, $this.objects);
            }
    ...
    _interpreterBinaryFile(_rawHex, _callback) {
        ...
        $this._interpreterCourse(_rawHex, function(eventInterpreter) {
            if(eventInterpreter) {
                ...
                for (let i = 0; i < $this.course.objectCount; i++) {
                    let pos = 0xF0 + (32 * i);
                    let ObjectCourse = $this._interpreterObject(_rawHex, pos);
                    $this.objects.push(ObjectCourse);
                }
                _callback(true);
            }
            else {
                console.error('SmmCourseViewer: Error: Interpret the binary!');
                _callback(false);
            }
        });
    }
     ...
     * @return {Object}
     */
    _interpreterObject(_rawHex, _pos) {
        /* interpreter */
        ...
        /* course object */
        var courseObject = new CourseObject(x, z, y, width, height, flags, childFlags, extendedData,
                type, childType, linkId, effect, transform, childTransform);
        return courseObject;
    }
}